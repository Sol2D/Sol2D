...
{
    auto it = m_objects_by_name.find(_name);
    return it == m_objects_by_name.cend() ? nullptr : it->get();
}

const TileMapObject * ObjectHeap::findBasicObject(uint32_t _gid) const
{
    auto it = m_objects_by_id.find(_gid);
    return it == m_objects_by_id.cend() ? nullptr : it->get();
}

void ObjectHeap::forEachObject(uint32_t _layer_id, std::function<void(const TileMapObject &)> _cb) const
{
    auto range = m_objects_by_layer.equal_range(_layer_id);
...
{
    auto range = m_objects_by_class.equal_range(_class);
    for(auto it = range.first; it != range.second; ++it)
        _cb(*it->get());
}
